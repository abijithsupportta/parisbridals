...
export function useBanners(params?: BannerSearchParams) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiFetch<{ banners: Banner[] }>(url);
...
export function useBannerCounts() {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiFetch<{ counts: Record<BannerType, number> }>('/api/banners/counts');
      return response.counts || { hero: 0, editorial: 0, split: 0 };
    },
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
    refetchOnWindowFocus: false,
...
export function useRemainingSlots() {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiFetch<{ remainingSlots: Record<BannerType, number> }>('/api/banners/remaining-slots');
      return response.remainingSlots || { hero: 10, editorial: 1, split: 2 };
    },
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
    refetchOnWindowFocus: false,
